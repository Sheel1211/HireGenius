...
const Main = () => {
  ...
  return (
    <div>
      <h1>CSV PARSER</h1>
      <input   onChange={(e)=>{setCsv(e.target.files[0])}} type ="file" accept=".csv" style={{ marginLeft: "4px" }}/>
      {console.log("csv",csv)}

       <button onClick={handleReadCSV}>Export Data from CSV</button>
    ...
    {candidates.map(can => (
      ...
      <p key={can.email}>Name: {can.name}, Email: {can.email}</p>
    ))}
  </>}
    </div>
  );
