    ...
    await Promise.all(
      candidates.map(async (item, index) => {
        ...
        const emailToCandi = await sendEmail(
          email,
          "Login Credentials",
          `Hii ${name}.<h2> your username:${username}</h2><h2>password:${password}</h2><h2>Test Link:${aptLink}?interviewId=${interviewId}</h2>`
        );
...
export const sendEmailToAllCandidatesForAPT = async (req, res) => {
  try {
