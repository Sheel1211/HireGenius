...
export const scheduleMeet = async (req, res) => {
  ...
  try {
    ...
    if (!mentor) {
      return res
        .status(400)
        .json({ success: false, message: "Mentor is not added to DB" });
    } else {
      const mentorData = await candidate.findOne({
        name: "mentor",
        email: mentorEmail,
        password: passwordOfMentor,
        username: mentorEmail.split("@")[0],
      ...
      if (!mentorData) {
        return res
          .status(400)
          .json({ success: false, message: "Mentor is not find in DB " });
      } else {
        const newGD = new gd({
          topic,
          duration,
        ...
        const sendEmailToMentor = await sendEmail(
          mentorData.email,
          "Mentor Credentials for Meet",
          `<h2>Username:${mentorData.username}</h2>
    ...
    res.status(200).json({
      success: true,
      message: "Email sended to all the Candidates & Mentor.",
    });
  } catch (err) {
